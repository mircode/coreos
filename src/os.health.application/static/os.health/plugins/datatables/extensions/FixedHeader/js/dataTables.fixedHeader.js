...
FixedHeader.prototype = {
	...
	_clone: function ( item, force )
	{
		...
		var itemElement = item === 'header' ?
			this.dom.thead :
			this.dom.tfoot;
		var itemBody=this.dom.tbody.clone(false);
		...
		if ( ! force && itemDom.floating ) {
			...
			var table_responsive=$(itemDom.host).parents('.table-responsive');
			if(table_responsive.length){
				var fixed_holder=$('.fixed-holder').length>0?
											$('.fixed-holder'):
											$('<div class="fixed-holder" style="width:'+table_responsive.width()+'px;">').appendTo(table_responsive);
			}
			var id=$(dt.table().node().cloneNode( false )).attr('id');
			itemDom.floating = $( dt.table().node().cloneNode( false ) )
				.attr('id',id+'_fixhead')
				.append( itemElement )
				.append( itemBody )
				.appendTo( fixed_holder||'body' );
