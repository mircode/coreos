...
+function(baseurl){
	...
	function search(){
		...
		// query
		table=$.JsonRPC('guard/query',param).done(function(result){
			...
			var header=[
						  ...
						  {title:'城市',data:'city'},
						  {title:'工作',data:'profession'},
						  {title:'关系',data:'relatives'},
						  {title:'时间',data:'time'}
						];
	...
	$('#btn_add').on('click',function(){
		$('#editForm').clear();
		$('#lay_pop').pop({title:'添加亲属',height:'360px'});
	});
	...
	$('#btn_edit').on('click',function(){
		...
		if(table!=null){
			...
			$.JsonRPC('guard/queryById',{table:action,id:id}).done(function(result){
				...
				var item=result;
				$('#editForm').form(item);
				$('#lay_pop').pop({title:'修改亲属',height:'360px'});	
			});
		}
	});
